...
- **Use a real graph.** Load even a small OpenStreetMap extract so edges represent actual roads. Euclidean distance ignores rivers, one-ways, and highways — fine for a demo, wrong for "accurate travel times."
- **Do A\* properly.** A real **priority queue** plus an **admissible heuristic** (straight-line or haversine distance, which never overestimates). Note *why* it's admissible — that's the property that guarantees A\* stays optimal.
- **Reconstruct paths.** Return the actual route, not just its cost, and validate that A\* and Dijkstra produce the *same cost* on the same pairs (A\* should just be faster).
- **_(stretch)_ Precompute/cache** distances for frequently-used source nodes (depots, hotspots).
- ✅ **Checkpoint:** given two points on the road graph, you get an optimal route and its travel time, matching Dijkstra's cost.

#### Week 5 · Aug 6, 2026 · Advanced Testing — ⬜ Not started
...
- **Real test framework.** Move off ad-hoc `main()` checks to **GoogleTest or Catch2**, wired into CTest so `ctest` runs everything.
- **Edge cases on purpose.** Empty input, a single point, all-coincident points, points exactly on boundaries, and N≠M — these are where spatial and matching code breaks.
- **Benchmark that *asserts*.** Don't just print a time; assert the pairing matrix computes within your sub-millisecond target for a stated input size, and **record the actual number** in the repo so regressions are visible.
- **_(stretch)_ Sanitizers.** Run the suite under **AddressSanitizer / UBSan** in CI to catch memory and undefined-behavior bugs the compiler silently allows.
- ✅ **Checkpoint:** `ctest` is green, edge cases are covered, and the sub-ms claim is backed by a committed measurement.
